...
/**
 * @author msx7 <br/>
 *         1、必须在构造函数或者{@link #setOnRefreshListener(PushHeader.OnRefreshListener)} 添加下拉刷新的
 *         监听，否则下拉刷新不起作用<br/>
 *         2、请通过{@link #getHeader()}获得添加至
 *         {@link ListView#addHeaderView(View, Object, boolean)} 或者添加至
 *         {@link android.widget.GridView}的上方， 3、下拉刷新结束请务必调用{@link #onRefreshComplete()}
...
public final class PushHeader extends AbstractHeader implements OnTouchListener {
    ...
    public PushHeader(AbsListView listView) {
        this(listView, null);
    }

    public PushHeader(AbsListView listView, OnRefreshListener l) {
        this(listView, l, true);
    }

    public PushHeader(AbsListView listView, OnRefreshListener l, boolean isAdd) {
        super(listView, true);
        ...
        mHeaderView = LayoutInflater.from(getContext()).inflate(R.layout.push_list_header, null);
        measureView(mHeaderView);
        headContentHeight = mHeaderView.getMeasuredHeight();
        if (isAdd && listView instanceof ListView)
            ((ListView) listView).addHeaderView(getHeader(), null, false);
...
//                mLastUpdatedTextView.setVisibility(View.VISIBLE);
                ...
                break;
            case DONE:
                if (getHeader() instanceof GridView)
                    getHeader().setPadding(0, -2 * headContentHeight, 0, 0);
                else
                    getHeader().setPadding(0, 0, 0, -2 * headContentHeight);
                mProgressBar.setVisibility(View.GONE);
                mArrowImageView.setVisibility(View.GONE);
                mArrowImageView.clearAnimation();
